use crate::{
    assets::GameAssets, audio::GameAudio, cleanup, component_adder::AnimationLink, game_camera,
    game_state, ingame, maze, menus, other_persons, ui::text_size, AppState,
};
use bevy::prelude::*;
...
impl Plugin for InGameUIPlugin {
    fn build(&self, app: &mut App) {
        app.add_system_set(SystemSet::on_exit(AppState::ResetInGame).with_system(setup))
            ...
            .add_system_set(
                SystemSet::on_update(AppState::InGame)
                    ...
                    .with_system(handle_textbox_events), //.with_system(detect_round_over),
            );
    }
}
...
fn setup(
            ...
            parent
                .spawn_bundle(NodeBundle {
                    style: Style {
                        size: Size::new(
                            Val::Percent(200.0),
                            Val::Px(scale * ingame::RENDER_TEXTURE_SIZE as f32),
                        ),
                        margin: UiRect {
    ...
    let scale = (text_scaler.window_size.width * 0.1) / ingame::RENDER_TEXTURE_SIZE as f32;
    let sprite_x =
        -(text_scaler.window_size.width / 2.0) + (ingame::RENDER_TEXTURE_SIZE as f32 / 2.0 * scale);
    let sprite_y =
        (text_scaler.window_size.height / 2.0) - (ingame::RENDER_TEXTURE_SIZE as f32 / 2.0 * scale);
    commands
...
fn display_textbox(
    ...
    if textbox.after_text_displayed_cooldown > 0.0 {
        return;
    }
                    ...
                    for will_animation_link in &will_animation_link {
                        if let Some(animation_entity) = will_animation_link.entity {
                            ...
                            if current_animation != current_text.animation_clip {
                                animation
                                    .play(current_text.animation_clip.clone_weak())
                                    .repeat();
                                animation.resume();
